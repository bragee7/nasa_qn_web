...
export default function StudentDashboard(){
  ...
  const [me,setMe]=useState<Student|undefined>(undefined);
  const [exams,setExams]=useState<Exam[]>([]);
  const [myAttempts,setMyAttempts]=useState<Attempt[]>([]);
  const [examNames,setExamNames]=useState<Record<string,string>>({});
  useEffect(()=>{ (async()=>{
    const students=await repo.all<Student>('students');
    const s=students.find(x=>x.id===session?.studentId); setMe(s);
    const all=await repo.all<Exam>('exams');
    setExams(all.filter(e=>['SCHEDULED','ACTIVE'].includes(e.status))
      .filter(e=>{ const ds=examDepartments(e); return (ds.length===0||ds.includes(s?.department??''))&&(e.year==='all'||e.year===s?.year)&&(e.section==='all'||e.section===s?.section); }));
    setMyAttempts(await repo.query<Attempt>('attempts',a=>a.uid===session?.uid));
    const names:Record<string,string>={}; for(const e of all) names[e.id]=e.title; setExamNames(names);
  })(); },[session?.studentId, session?.uid]);
  const nowT=Date.now();
  ...
  return <Shell sidebar={<><SideLink to="/student/dashboard" label="Dashboard" /><SideLink to="/student/exams" label="My Exams" /><SideLink to="/student/profile" label="Profile" /></>}>
    ...
    {myAttempts.filter(a=>a.status!=='IN_PROGRESS').length===0?<Empty title="No completed exams yet." />:myAttempts.filter(a=>a.status!=='IN_PROGRESS').map(a=>{
      return <Card key={a.id}><b>{examNames[a.examId] ?? a.examId}</b><p className="text-sm">Score: {a.score} / {a.totalMarks} ({a.pct}%)</p><Link className="btn-ghost mt-2" to={`/student/result/${a.id}`}>View score</Link></Card>;})}
  </Shell>;
}
